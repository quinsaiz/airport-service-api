...
def sample_flight(**params: Any) -> Flight:
    if "route" not in params:
        ...
        airport_2 = Airport.objects.create(name="Destination", closest_big_city="City 2")
        route = Route.objects.create(source=airport_1, destination=airport_2, distance=100)
        params["route"] = route

    if "airplane" not in params:
        airplane_type = AirplaneType.objects.create(name="Cargo")
        airplane = Airplane.objects.create(name="An-225", rows=10, seats_in_row=5, airplane_type=airplane_type)
